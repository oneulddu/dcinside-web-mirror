...
def video_size(video_id):
    ...
    size = probe_frame0_size(video_id) or probe_shorts_orientation(video_id)
...
def sizes_for_ids(raw_ids):
    ...
    for raw_id in raw_ids:
        ...
        if len(unique_ids) >= SIZE_MAX_IDS_PER_REQUEST:
            break
    return {video_id: video_size(video_id) for video_id in unique_ids}
