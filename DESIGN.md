...
2026-09-19 홈 개선은 `body.home-view`와 `.home-galleries`에 한정한다.
게시판, 읽기, 최근 화면은 기존 토큰과 컴포넌트 규칙을 유지한다.

- 홈 전용 토큰: `--home-line`은 라이트 `#DCE1E7` / 다크 `#3B3B43`,
  `--home-accent`는 `#1D64D8` / `#78AAFA`, `--home-muted`는 `#596574` / `#A1A1AA`.
- 검색 전후 간격은 24px. 입력은 16px 글자와 최소 44px 높이, 검색 버튼은 채움형이다.
  홈의 테마·이미지 설정 버튼은 44px 정사각, radius 8px다.
- 순위 열은 28px 고정폭에 오른쪽 정렬하고, 12px 간격 뒤에 제목과 ID를 함께 배치한다.
  행은 최소 64px 높이이며 긴 이름과 ID는 줄바꿈한다. 등급은 12px 텍스트 라벨이다.
- 480px 이하에서는 섹션 제목과 갱신 시각을 4px 간격으로 두 줄에 배치한다.
  380px 이하에서는 브랜드 보조 문구를 숨겨 주요 조작부의 공간을 확보한다.
- 검색 결과가 없으면 검색용 빈 문구를 표시하고, 외부 갤러리는 `새 탭 이동`을 표시한다.
- 평가와 검증 기록: `docs/home-design-review-2026-09-19.md`.
