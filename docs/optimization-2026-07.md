...
|---|---|---|---|---|
| P1-1 `/board` 목록 캐시 | P1 | 리스트뷰 요청 대폭 절감 | 낮음 (기존 캐시 패턴 재사용) | 완료 |
| P1-2 정적 자산 압축 | P1 | 첫 방문 전송량 ~80% 절감 | 매우 낮음 | 완료 |
| P1-3 `/read` 단기 캐시 | P1(선택) | 게시글 요청 절감 | 중간 (댓글 최신성) | 완료 (기본 비활성) |
| P2-1 워커 수 재검토 | P2 | 캐시 적중률 상승 | 낮음 (운영 설정) | 완료 |
| P2-2 검색 결과 캐시 | P2 | 검색 요청 절감 | 낮음 | 완료 |
| P2-3 BS4 파서 통일 | P2 | 홈/검색 파싱 CPU 절감 | 낮음 (테스트로 확인) | 완료 |
| P2-4 이중 파싱 계측 | P2 | 미정 (계측 우선) | — | 보류 (별도 측정 필요) |
| P3-1 폰트 preconnect | P3 | 첫 렌더 소폭 개선 | 매우 낮음 | 완료 |
| P3-2 정적 로그 제외 | P3 | 로그 볼륨 절감 | 매우 낮음 | 완료 |
| P3-3 prune 비용 절감 | P3 | 락 경합 소폭 완화 | 낮음 | 완료 |
| P3-4 캐시 상수 정리 | P3 | 가독성 | 없음 | 완료 |
